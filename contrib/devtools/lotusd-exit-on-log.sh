...
DEFAULT_BITCOIND="${TOPLEVEL}/build/src/lotusd"
DEFAULT_LOG_FILE=~/".bitcoin/debug.log"

help_message() {
  set +x
  echo "Run lotusd until a given log message is encountered, then kill lotusd."
  echo ""
  echo "Example usages:"
  ...
  echo ""
  echo "-c, --callback        (optional) Bash command to execute as a callback. This is useful for interacting with lotusd before it is killed (to run tests, for example)."
  echo "-p, --params          (optional) Parameters to provide to lotusd."
  echo ""
...
# Launch lotusd using custom parameters
read -a BITCOIND_PARAMS <<< "${BITCOIND_PARAMS}"

BITCOIND_PID_FILE=/tmp/lotusd-exit-on-log.pid
...
while [ ! -e "${BITCOIND_PID_FILE}" ]; do
  ...
  if [ "${PID_WAIT_COUNT}" -gt 10 ]; then
    echo "Timed out waiting for lotusd PID file"
    exit 10
  fi
  sleep 0.5
done
...
(
  # When this subshell finishes, kill lotusd
  log_subshell_cleanup() {
    echo "Cleaning up lotus daemon (PID: ${BITCOIND_PID})."
    kill ${BITCOIND_PID}
  }
  ...
  # Optional callback for interacting with lotusd before it's killed
  if [ -n "${CALLBACK}" ]; then
    "${CALLBACK}"
  fi
) &
...
# Wait for lotusd to exit, whether it exited on its own or the log subshell finished
set +x
...
if [ -e /proc/${LOG_PID} ]; then
  echo "lotusd exited unexpectedly. See '${LOG_FILE}' for details."
  exit 20
fi
