...
def create_transaction(node, txid, vout, to_address, *, amount):
...
def spend_tx(node, prev_tx, address):
    spendtx = create_transaction(
        node, prev_tx.txid_hex, 0, address, amount=(prev_tx.vout[0].nValue - 1000) / COIN)
...
class BIP68_112_113Test(BitcoinTestFramework):
    ...
    def send_blocks(self, blocks, success=True):
        ...
        self.nodes[0].p2p.send_blocks_and_test(
            blocks, self.nodes[0], success=success, timeout=15)
