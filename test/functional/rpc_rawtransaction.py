...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.sync_all()
        self.nodes[0].sendtoaddress(self.nodes[2].getnewaddress(), Decimal('20'))
        self.nodes[0].sendtoaddress(self.nodes[2].getnewaddress(), Decimal('50'))
        self.nodes[0].sendtoaddress(self.nodes[2].getnewaddress(), Decimal('100'))
        self.sync_all()
        ...
        if not self.options.descriptors:
            ...
            assert_equal(self.nodes[0].getbalance(),
                         bal + SUBSIDY + Decimal('219.000000'))  # block reward + tx
            ...
            # block reward + tx
            assert_equal(self.nodes[0].getbalance(),
                         bal + SUBSIDY + Decimal('219.000000'))
        ...
        txid = self.nodes[0].sendtoaddress(addr, 1000)
        self.nodes[0].generate(1)
        self.sync_all()
        vout = find_vout_for_address(self.nodes[1], txid, addr)
        rawTx = self.nodes[1].createrawtransaction(
            [{'txid': txid, 'vout': vout}],
            {self.nodes[1].getnewaddress(): Decimal("999.9")})
        ...
        # and the following calls should both succeed
        testres = self.nodes[2].testmempoolaccept(
            rawtxs=[rawTxSigned['hex']], maxfeerate='20.000000')[0]
        assert_equal(testres['allowed'], True)
        self.nodes[2].sendrawtransaction(
