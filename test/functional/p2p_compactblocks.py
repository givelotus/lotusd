...
class CompactBlocksTest(BitcoinTestFramework):
    ...
    # This test actually causes lotusd to (reasonably!) disconnect us, so do
    # this last.
    ...
    def check_compactblock_construction_from_block(
        ...
        for entry in header_and_shortids.prefilled_txn:
            ...
            assert_equal(entry.tx.txid, block.vtx[entry.index].txid)
            assert_equal(entry.tx.txhash, block.vtx[entry.index].txhash)
        ...
        while index < len(block.vtx):
            if (len(header_and_shortids.prefilled_txn) > 0 and
                ...
                txhash = block.vtx[index].txhash
                shortid = calculate_shortid(k0, k1, txhash)
                assert_equal(shortid, header_and_shortids.shortids[0])
                header_and_shortids.shortids.pop(0)
            index += 1

    # Test that lotusd requests compact blocks when we announce new blocks
    ...
    def test_compactblock_requests(self, node, test_node, version):
        ...
        for announce in ["inv", "header"]:
            ...
            coinbase_hash = block.vtx[0].txhash
            comp_block.shortids = [
    ...
    def test_getblocktxn_requests(self, node, test_node, version):
        ...
        self.utxos.append(
            [ordered_txs[-1].txid, 0, ordered_txs[-1].vout[0].nValue])
        comp_block = HeaderAndShortIDs()
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for lotusd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, node, test_node, version):
        # lotusd will not send blocktxn responses for blocks whose height is
            ...
            with p2p_lock:
                ...
                for index in all_indices:
                    tx = test_node.last_message["blocktxn"].block_transactions.transactions.pop(
                        0)
                    tx.rehash()
                    assert_equal(tx.txid, block.vtx[index].txid)
                    assert_equal(tx.txhash, block.vtx[index].txhash)
    ...
    def test_compactblock_reconstruction_multiple_peers(
        ...
        self.utxos.append(
            [block.vtx[-1].txid, 0, block.vtx[-1].vout[0].nValue])
