...
class InventoryDownloadTest(BitcoinTestFramework):
    ...
    def test_inv_tx(self, context):
        ...
        tx = self.nodes[0].createrawtransaction(
            ...
            outputs={ADDRESS_ECREG_UNSPENDABLE: SUBSIDY - Decimal('0.025')},
        )
        ...
        msg = msg_inv([CInv(t=context.inv_type, h=ctx.txid)])
        for p in self.peers:
            p.send_and_ping(msg)
        ...
        with self.nodes[1].assert_debug_log([f"got inv: tx {ctx.txid:064x}  new peer=0"]):
