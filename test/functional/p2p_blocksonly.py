...
from test_framework.blocktools import SUBSIDY, create_transaction
...
class P2PBlocksOnly(BitcoinTestFramework):
    ...
    def check_p2p_tx_violation(self, index=1):
        ...
        input_txid = self.nodes[0].getblock(
            self.nodes[0].getblockhash(index), 2)['tx'][0]['txid']
        tx = create_transaction(
            self.nodes[0],
            input_txid,
            self.nodes[0].getnewaddress(),
            amount=SUBSIDY - Decimal('0.1'),
            vout=1)
        tx.rehash()
        txid = tx.txid_hex
