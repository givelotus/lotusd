...
* Manage regtest lotusd subprocesses.
* Access RPC interfaces of the underlying lotusd instances.
...
The following `TestShell` methods manage the lifetime of the underlying lotusd
...
temporary folder. If you need more lotusd nodes than set by default (1),
...
During the time between the `setup` and `shutdown` calls, all `lotusd` node
...
useful in constructing objects which can be passed to the lotusd nodes managed
by a running `TestShell` object.

## 5. Shutting the `TestShell` down

Shutting down the `TestShell` will safely tear down all running lotusd
...
The following utility consolidates logs from the lotusd nodes and the
...
|---|---|---|
| `bind_to_localhost_only` | `True` | Binds lotusd RPC services to `127.0.0.1` if set to `True`.|
| `cachedir` | `"/path/to/test/cache"` | Sets the lotusd datadir directory. |
| `chain`  | `"regtest"` | Sets the chain-type for the underlying test lotusd processes. |
| `configfile` | `"/path/to/test/config.ini"` | Sets the location of the test framework config file. |
| `coveragedir` | `None` | Records lotusd RPC test coverage into this directory if set. |
| `loglevel` | `INFO` | Logs events at this level and higher. Can be set to `DEBUG`, `INFO`, `WARNING`, `ERROR` or `CRITICAL`. |
| `nocleanup` | `False` | Cleans up temporary test directory if set to `True` during `shutdown`. |
| `noshutdown` | `False` | Does not stop lotusd instances after `shutdown` if set to `True`. |
| `num_nodes` | `1` | Sets the number of initialized lotusd processes. |
| `perf` | False | Profiles running nodes with `perf` for the duration of the test if set to `True`. |
| `rpc_timeout` | `60` | Sets the RPC server timeout for the underlying lotusd processes. |
| `setup_clean_chain` | `False` | Initializes a 200-block-long chain by default. If set to `True`, initialize an empty chain. |
| `randomseed` | Random Integer | `TestShell.options.randomseed` is a member of `TestShell` which can be accessed during a test to seed a random generator. User can override default with a constant value for reproducible test runs. |
| `supports_cli` | `False` | Whether the lotus-cli utility is compiled and available for the test. |
| `tmpdir` | `"/var/folders/.../"` | Sets directory for test logs. Will be deleted upon a successful test run unless `nocleanup` is set to `True` |
| `trace_rpc` | `False` | Logs all RPC calls if set to `True`. |
| `usecli` | `False` | Uses the lotus-cli interface for all lotusd commands instead of directly calling the RPC server. Requires `supports_cli`. |
