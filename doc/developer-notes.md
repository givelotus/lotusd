...
Lotus is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_lotus
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_lotus --log_level=test_suite
$ valgrind -v --leak-check=full src/lotusd -printtoconsole
```
...
To profile a running lotusd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep lotusd` -- sleep 60
```
...
Lotus can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`lotusd`)](https://www.bitcoinabc.org/doc/dev/bitcoin_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `lotusd.cpp`. Responsible for starting up and
...
However, in open source software such as Lotus, where everyone uses
...
pay attention to for reviewers of Lotus code.
...
submitted to Lotus so that they can be integrated quickly.
...
- src/leveldb
  ...
  - **Note**: Follow the instructions in [Upgrading LevelDB](#upgrading-leveldb)
    when merging upstream changes to Lotus.

- src/secp256k1
  - Upstream at <https://github.com/bitcoin-core/secp256k1/> ; actively maintained
    by Bitcoin Core contributors.
    Lotus is using a modified version of libsecp256k1, some changes might
    be directly submitted to Lotus.
...
would be to revert the upstream fix before applying the updates to Lotus's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `lotus-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `lotus-cli` and the GUI debug console use this table to determine how to
