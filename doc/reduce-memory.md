...
There are a few parameters that can be dialed down to reduce the memory usage of `lotusd`. This can be useful on embedded systems or small VPSes.
...
By default, since glibc `2.10`, the C library will create up to two heap arenas per core. This is known to cause excessive memory usage in some scenarios. To avoid this make a script that sets `MALLOC_ARENA_MAX` before starting `lotusd`:
